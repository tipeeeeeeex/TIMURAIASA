...
const CheckOptions: React.FC<CheckOptionsProps> = function CheckOptions(props) {
  ...
  const handleShare = (event: React.MouseEvent<HTMLDivElement | HTMLLIElement, MouseEvent>) => {
    ...
    Toast.info('Ova stavka je u fazi izrade.');
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex max-[408px]:hidden">
        <OptionButton type="secondary" text="Novi razgovor" icon={<IconPlus />} onClick={onCreateNewChat} />
        <OptionButton type="danger" text="Obriši" icon={<IconDelete />} onClick={onDeleteRecord} />
        <OptionButton type="tertiary" text="Podjeli" icon={<IconShareStroked />} onClick={handleShare} />
      </div>
      <Dropdown clickToHide stopPropagation trigger="click" content={renderContent}>
        <div className="hidden max-[408px]:block">
          <Button type="tertiary" icon={<IconList />}>Opcije</Button>
        </div>
      </Dropdown>
