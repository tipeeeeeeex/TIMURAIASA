...
const Chat: React.FC<ChatProps> = function Chat(props) {
  ...
  const handleSelectList = () => {
    if (loading) Toast.warning('Molimo sacekajte da se ova konverzacija završi.');
  ...
  const handleCreateChat = (event: React.MouseEvent<HTMLDivElement | HTMLLIElement, MouseEvent>) => {
    ...
    if (checkList.length === 0) {
      Toast.warning('Molimo odaberite bar 1 opciju.');
    } else {
  ...
  const renderRetryButton = () => {
    ...
    return (
      ...
      >
        ...
        {`${loading ? 'Zaustavi odgovor' : 'Traži odgovor opet'}`}
      </button>
    );
  };
